...
## 5. Déclencher une menace pour la démo (3 scénarios disponibles)

```powershell
# Brute-force (tentatives de login échouées)
docker exec attack-simulator touch /tmp/trigger_bruteforce

# Scan de reconnaissance (sondage de plusieurs endpoints)
docker exec attack-simulator touch /tmp/trigger_scan

# Pic de volume / DDoS léger
docker exec attack-simulator touch /tmp/trigger_ddos
```

Chaque scénario se déclenche **immédiatement** (pas d'attente), dure quelques
secondes, et tu peux le **relancer autant de fois que tu veux** avec la même
commande — pratique pour répéter la démo devant ton prof.

Le système classera automatiquement le type de menace, proposera une
contre-mesure adaptée, et affichera une IP source **simulée à titre
illustratif** (les métriques Prometheus n'exposent pas d'IP réelle — c'est
expliqué clairement dans l'interface et le rapport PDF, par souci de rigueur
académique).
